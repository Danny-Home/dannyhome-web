...
export const productsRouter = createTRPCRouter({
  ...
  bySlug: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const withImgs = await includeAttachmentsForEntity(ctx.db, STORAGE_KEYS.PRODUCT, product);
      return withImgs;
    }),

  byCategorySlug: publicProcedure
    .input(z.object({ slug: z.string(), page: z.number().optional(), perPage: z.number().optional() }))
    .query(async ({ ctx, input }) => {
      ...
      const items = await ctx.db.product.findMany({
        ...
        skip: input.page && input.perPage ? (input.page - 1) * input.perPage : 0
      });
      const products = await includeAttachmentsForEntity(ctx.db, STORAGE_KEYS.PRODUCT, items);
